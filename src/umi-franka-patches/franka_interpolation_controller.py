...
class FrankaInterface:
    ...
    def move_to_joint_positions(self, positions: np.ndarray, time_to_go: float):
        self.server.move_to_joint_positions(
            [float(x) for x in np.asarray(positions).flat],
            float(time_to_go),
        )

    def start_cartesian_impedance(self, Kx: np.ndarray, Kxd: np.ndarray):
        self.server.start_cartesian_impedance(
            [float(x) for x in np.asarray(Kx).flat],
            [float(x) for x in np.asarray(Kxd).flat],
        )
    
    def update_desired_ee_pose(self, pose: np.ndarray):
        self.server.update_desired_ee_pose(
            [float(x) for x in np.asarray(pose).flat]
        )
...
class FrankaInterpolationController(mp.Process):
    ...
    def stop(self, wait=True):
        ...
        if self.is_alive():
            try:
                ...
                if self.verbose:
                    print("[FrankaPositionalController] Stop queue full; forcing shutdown.")
        if wait:
            self.stop_wait()
    ...
    def start_wait(self):
        ...
        if not ready:
            raise TimeoutError(
                f"FrankaPositionalController did not become ready within {self.launch_timeout}s."
            )
        assert self.is_alive()
    
    def stop_wait(self):
        self.join(timeout=max(1.0, self.launch_timeout))
        if self.is_alive():
            if self.verbose:
                print("[FrankaPositionalController] Stop timed out; terminating process.")
            self.terminate()
    ...
    def _put_command(self, message):
        if not self.is_alive():
            raise RuntimeError("Franka controller process is not running.")
        try:
            self.input_queue.put(message)
        except Full:
            if not self.is_alive():
                raise RuntimeError("Franka controller process is not running.") from None
            raise RuntimeError("Franka controller input queue is full.") from None
    ...
    def move_to_joint_positions(self, positions, time_to_go):
        ...
        message = {
            'cmd': Command.MOVE_TO_JOINT_POSITIONS.value,
            'target_pose': np.zeros((6,), dtype=np.float64),
            'duration': float(time_to_go),
            'target_time': 0.0,
            'target_joints': positions,
        }
        self._put_command(message)
    ...
    def run(self):
        ...
        cartesian_policy_started = False
        ...
        try:
            ...
            while keep_running:
                ...
                for i in range(n_cmd):
                    ...
                    elif cmd == Command.MOVE_TO_JOINT_POSITIONS.value:
                        target_joints = np.asarray(command['target_joints'], dtype=np.float64)
                        time_to_go = float(command['duration'])

                        if cartesian_policy_started:
                            robot.terminate_current_policy()
                            cartesian_policy_started = False

                        robot.move_to_joint_positions(target_joints, time_to_go)
                        curr_pose = robot.get_ee_pose()
            ...
            if cartesian_policy_started:
                try:
                    ...
                    if self.verbose:
                        print(f"[FrankaPositionalController] terminate_current_policy failed: {exc}")
            del robot
            self.ready_event.set()
